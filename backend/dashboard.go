...
// pricePoller polls the min-api.cryptocompare REST API for SHI token value.
	...
	go func() {
		...
		for {
			...
			case <-a.killSignal:
				return
			default:
				a.wallet.TokenPrice.SHI.USD = 0
				a.wallet.TokenPrice.SHI.EUR = 0
				a.wallet.TokenPrice.SHI.BTC = 0
				time.Sleep(time.Duration(retryCounter) * time.Second) // Incremental backoff
				for retryCounter <= 20 && a.wallet.Balance != 0 {
					...
					if a.wallet.Balance != 0 && a.wallet.TokenPrice.SHI.USD == 0 {

						a.log.Infoln("Contacting alternate token evaluation API")
						a.wallet.TokenPrice.SHI.USD, a.wallet.TokenPrice.SHI.BTC, err = getTokenPriceAlternateRoute()
					...
					a.log.Infof("Collected token price in USD: %v", a.wallet.TokenPrice.SHI.USD)
					a.log.Infof("Collected token price in EUR: %v", a.wallet.TokenPrice.SHI.EUR)
					a.log.Infof("Collected token price in BTC: %v", a.wallet.TokenPrice.SHI.BTC)

					totalCurrencyBalance := 0.0
					if a.wallet.Currency == "USD" {
						totalCurrencyBalance = float64(a.wallet.Balance) * a.wallet.TokenPrice.SHI.USD
					} else if a.wallet.Currency == "EUR" {
						totalCurrencyBalance = float64(a.wallet.Balance) * a.wallet.TokenPrice.SHI.EUR
					} else if a.wallet.Currency == "BTC" {
						totalCurrencyBalance = float64(a.wallet.Balance) * a.wallet.TokenPrice.SHI.BTC
					}
