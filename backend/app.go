	...
	Network    struct {
		...
		BlockExplorer struct {
			...
			Handles struct {
				...
				CollectTX    string // Takes SHI address, returns tx objects
...
func (a *WalletApplication) WailsInit(runtime *wails.Runtime) error {
	...
	a.DB, err = gorm.Open("sqlite3", a.paths.SHIDir+"/store.db")
	if err != nil {
...
// initLogger writes logs to STDOUT and a.paths.SHIDir/wallet.log
func (a *WalletApplication) initLogger() {
	logFile, err := os.OpenFile(a.paths.SHIDir+"/wallet.log", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0664)
	if err != nil {
...
func (a *WalletApplication) initDirectoryStructure() error {
	...
	a.paths.SHIDir = a.paths.HomeDir + "/.shi" // SHI directory for configuration files and wallet specific data
	a.paths.TMPDir = a.paths.SHIDir + "/tmp"
	...
	a.log.Info("SHI Directory: ", a.paths.SHIDir)

	err = a.directoryCreator(a.paths.SHIDir, a.paths.TMPDir)
	if err != nil {
		return err
	}
...
func (a *WalletApplication) initMainnetConnection() {
	a.Network.URL = "http://lb.constellationnetwork.io:9000" // Указать адрес сети

	a.Network.Handles.Send = "/send"
	a.Network.Handles.Transaction = "/transaction"
	a.Network.Handles.Balance = "/address/"

	a.Network.BlockExplorer.URL = "https://xju69fets2.execute-api.us-west-1.amazonaws.com/cl-block-explorer-mainnet" //Сеть
