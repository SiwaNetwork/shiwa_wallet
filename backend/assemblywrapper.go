...
by the ShiwaNetwork Engineering team. The code is interfacing with the wallet assembly using
the CLI */
...
func (a *WalletApplication) runWalletCMD(tool string, scalaFunc string, scalaArgs ...string) error {
	...
	cmds := []string{"-jar", a.paths.SHIDir + "/cl-" + tool + ".jar", scalaFunc}
...
func (a *WalletApplication) WalletKeystoreAccess() bool {
	...
	w.Close()
	shiAddress, err := ioutil.ReadAll(r)
	if err != nil {
		a.log.Errorln("Unable to read address from STDOUT", err)
		a.sendError("Unable to read address from STDOUT", err)
	}
	// if STDOUT prefix of show-address output isn't SHI

	if err == nil && a.wallet.Address != "" && string(shiAddress[:40]) == a.wallet.Address {
...
// GenerateSHIAddress generates a new wallet address and stores it in memory
...
func (a *WalletApplication) GenerateSHIAddress() string {
	a.log.Infoln("Creating SHI Address from Public Key...")
	...
	os.Stdout = rescueStdout
	a.wallet.Address = string(shiAddress[:40])

	return a.wallet.Address
}

// CheckAndFetchWalletCLI will download the cl-wallet dependencies from
// the official ShiwaNetwork Repo
func (a *WalletApplication) CheckAndFetchWalletCLI() bool {
	keytoolPath := a.paths.SHIDir + "/cl-keytool.jar"
	walletPath := a.paths.SHIDir + "/cl-wallet.jar"
...
// java -jar cl-wallet.jar create-transaction --keystore testkey.p12 --alias alias --storepass storepass --keypass keypass -d SHI6o9dcxo2QXCuJS8wnrR944YhFBpwc2jsh5j8f -p prev_tx -f new_tx --fee 0 --amount 1
