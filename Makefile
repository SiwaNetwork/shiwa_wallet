...
help:
	@printf "\nBelow commands have been made available to ease up Shiwa Walletdevelopment.\n\n"

	@printf "Shiwa WalletApp: \n"

	@printf "  $(bold)run_dev_env$(sgr0) - Starts up the dev enviornment on http://localhost:8080\n"
	@printf "  $(bold)build_app$(sgr0) - Compiles the Shiwa Walletto the build folder\n"
	...
	@printf "  $(bold)build_update_module$(sgr0) - Builds the update module to the ~/.shi folder\n"
...
build_app:
	@echo "Building Shiwa Walletbinary to build folder..."
	wails build
...
build_update_module:
	@echo "Building update module binary to ~/.shi"
	go build -o ~/.shi/update backend/cmd/update/main.go
