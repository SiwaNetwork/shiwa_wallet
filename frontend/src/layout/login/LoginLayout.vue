<template>
  <div id="app" class="login-bg vertical-center" v-bind:style="{ backgroundImage: 'url(' + themeBG + ')' }" >
    <div class="container">
      <div class="row">
        <div class="col mx-auto text-center header">
          <div>
            <img class="img-fluid" v-if="isDarkMode" src="~@/assets/img/Shiwa-Logo-White.png" style="max-height: 5.8rem;" />
            <img class="img-fluid" v-else src="~@/assets/img/Shiwa-Logo-Black.png" style="max-height: 5.8rem;" />
            <p v-if="this.$route.params.message">{{this.$route.params.message}}</p>
            <p v-else>Downloading $SHI wallet dependencies...</p>
          </div>
    ...
    <div class="version">
      ...
      Shiwa Wallet version: {{uiVersion}}</p>
    </div>
  </div>
</template>
