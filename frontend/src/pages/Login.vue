    ...
    newWallet: function() {
      this.$store.dispatch('wallet/reset').then(() => {
        this.$router.push({
          name: 'new wallet', 
          params: {
            message: "Create a new Shiwa wallet. Пожалуйста, убедитесь, что вы сохранили резервную копию всей информации, представленной ниже, в надежном месте.",
        ...
        if (result) {
          ...
          .then (result => {
            ...
            if (result) {
              self.$router.push({
                name: 'loading', 
                params: {message: "Getting your $SHI Wallet ready..."}
              });
            } else {
              self.$router.push({
